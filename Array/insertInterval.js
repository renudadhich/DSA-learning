...
// ascending order by starti and intervals still does not 
...
const insertInerval = (intervals, newInterval) => {
    let [first, last] = newInterval;
    let leftArr = [], rightArr = [];
    for(let index =0; index < intervals.length; index++){
        let [start, end] = intervals[index];

        if(end < first) {
            leftArr.push(intervals[index]);
        } else if(last < start) {
            rightArr.push(intervals[index])
        } else {       // there is a overlap
              first = Math.min(start, first);
              last = Math.max(end, last);
        }
    }
    return [...leftArr, [first, last], ...rightArr]
}

console.log(insertInerval([[1,2],[3,5],[6,7],[8,10],[12,16]], [4,8]));