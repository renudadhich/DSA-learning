...
function solution(U, weight) {
   ...
   let start = 0;
   let turnBackCount = 0;
   while(start < weight.length-1) {
       let sum = weight[start]+weight[start+1];

       if(sum > U) {
           if(weight[start] > weight[start+1]) {
               turnBackCount++;
               for(let j = start; j< weight.length-1; j++) {
                   weight[j] = weight[j+1];
               }
         
           } else {
                turnBackCount++;
               for(let j = start+1; j< weight.length-1; j++) {
                   weight[j] = weight[j+1];
               }
